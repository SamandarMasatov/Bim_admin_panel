      ...
      <v-form v-model="valid" class="formm"  @submit.prevent>
        <div class="grey lighten-2 pa-2 mb-4">
          ...
          <v-row>
            ...
            <v-col cols="12" sm="6">
                <div class="pb-2 ps-1">Familiya</div>
                <v-text-field
                    solo
                    v-model="familya"
                    :rules="Textarea"
            ...
            <v-col cols="12">
                <div class=" pb-2 ps-1">Lavozim kiriting (Masalan: Direktor, Ilmiy zavuch)</div>
                <v-text-field
                    solo
                    v-model="lavozim"
                    :rules="Textarea"
        ...
        <v-row>
          <v-col cols="12">
            <div class="pa-2 pb-0 grey lighten-2">
              <div class="mb-2">
...
export default {
  ...
  created() {
    this.$axios
      .get(`/team/getAll/${this.ID}`)
      .then((res) => {
        this.data = res.data
        this.ism = res.data.firstName;
        this.familya = res.data.lastName;
        this.lavozim = res.data.position;
  ...
  methods: {
    sendData() {
      if (this.image != null) {
        ...
        const data = {
          firstName: this.ism,
          lastName: this.familya,
          position: this.lavozim,
          description: this.description,
        }

        this.$axios
          .put(`/team/updateInfo/${this.ID}`, data)
          .then((res) => {
